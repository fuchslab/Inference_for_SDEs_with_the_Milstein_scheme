...
# collect the hyperparameters in a list (used in the creation of the output files)
hyperparam <- list(kappa_b = kappa_b, nu_b = nu_b, kappa_s = kappa_s,
                   nu_s = nu_s, gamma_b = gamma_b, gamma_s = gamma_s)
...
propose_theta <- function(theta){
  ...
  #alpha
  proposal_theta[1] <- alpha
  # beta
  proposal_theta[2] <- rlnorm(1, mean = log(theta[2]), sd = gamma_b)
  # sigma^2
  proposal_theta[3] <- rlnorm(1, mean = log(theta[3]), sd = gamma_s)
...
log_dprior_theta <- function(theta){
  ...
  prior_theta[1] <- 1
  # beta
  prior_theta[2] <- dinvgamma(theta[2], shape = kappa_b, scale = nu_b)
  # sigma^2
  prior_theta[3] <- dinvgamma(theta[3], shape = kappa_s, scale = nu_s)
...
dMBMilstein <- function(y, x_k, x_b, theta, delta_t, left_index, right_index,
                        precision = 10^4){
  if( any(c(y, x_k, x_b) < 0)){ # CIR-specific
    stop("The function dMBMilstein() is only valid for non-negative input states y, x_k, x_b.\n",
         " y = ", y,
         ", x_k = ", x_k,
         ", x_b = ", x_b
    )
  ...
  min_y2 <- theta[2] - 1/theta[1] * (x_b / (stepsToRight * delta_t) + theta[3] / 4)

  min_y <- max (0, min_y1, min_y2)
  max_y <- Inf
  ...
  if( min_y > max_y ){
    ...
    dMBMilstein_unnormalized <- function(y, x_k, x_b, theta, delta_t, stepsToRight, scale = 1){
      ...
      # CIR specific (!) implementation to avoid numerical issues:
      A_y_squared <- theta[3] * ( x_b - (theta[1] * (theta[2] - y) - theta[3]/4) * (stepsToRight * delta_t))
...
rMBMilstein <- function(n, x_k, x_b, theta, delta_t, left_index, right_index,
                        precision = 10^4){
  if( any(c(x_k, x_b) < 0)){ # CIR-specific
    stop("The function rMBMilstein() is only valid for non-negative input states x_k, x_b.\n",
         "x_k = ", x_k,
         ", x_b = ", x_b
    )
  }
  ...
  if( min_y > max_y ){
    ...
    dMBMilstein_unnormalized1 <- function(y, x_k, x_b, theta, delta_t, stepsToRight){
      ...
      # CIR specific (!) implementation to avoid numerical issues:
      A_y_squared <- theta[3] *
        ( x_b - (theta[1] * (theta[2] - y) - theta[3]/4) * (stepsToRight * delta_t))
      # due to numercial issues, A_y_squared may become negative (but small) instead of zero
      cond2 <- A_y_squared < 0 & abs(A_y_squared) < 1e-11
      if(any(cond2)){
        A_y <- vector(mode = "numeric", length = length(y))
